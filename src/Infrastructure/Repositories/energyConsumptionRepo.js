...
class EnergyConsumptionRepo {
  async getEnergyConsumptionByCompanyId(company_id) { // Updated method
    const energyData = await EnergyConsumptionSchema.find({ company_id }).lean();
    return energyData.length > 0 ? energyData[0] : { yearlyConsumption: 0, emissions: 0, country: null, company_id };
  }
